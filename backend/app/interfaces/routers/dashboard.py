...
async def get_dashboard(
    ...
    total_alunos = (await session.execute(stmt_alunos)).scalar() or 0
    ...
    stmt_reports = select(func.count(Report.id)).where(Report.travado == False)
    total_reports = (await session.execute(stmt_reports)).scalar() or 0
    ...
    total_fotos = (await session.execute(stmt_fotos)).scalar() or 0
