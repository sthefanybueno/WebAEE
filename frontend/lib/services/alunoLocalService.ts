 ...
 * ===================================
 * Serviço de domínio local para operações offline-first sobre Alunos.
 *
 * Responsabilidades:
 *  - Persistir alunos no IndexedDB (Dexie).
 *  - Enfileirar operações de sync com a prioridade correta.
 *
 * NUNCA instancie db diretamente em hooks ou componentes para
 * operações de escrita — use este serviço.
 *
...
import { db, enqueue, type AlunoLocal } from '@/lib/db'

/**
 * Salva um aluno localmente no IndexedDB e enfileira sincronização.
 *
 * Usa `enqueue()` de lib/db.ts, que conhece a regra de prioridade —
 * garantindo que alunos recebam prioridade 2 (não 1, que é de relatórios).
 *
...
/**
 * Atualiza um aluno local e enfileira sync de atualização.
 */
